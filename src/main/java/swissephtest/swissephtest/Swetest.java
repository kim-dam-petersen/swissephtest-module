...
"BCcg"+
  ...
  private StringBuilder serr_save=new StringBuilder();
  private StringBuilder serr_warn=new StringBuilder();
...
//ephepath="./ephe";
    if (ephepath.length() == 0) {
      StringBuilder sbEphepath = new StringBuilder();
...
//      if (sscanf (sp, "%d%*c%d%*c%d", &jday,&jmon,&jyear) < 1) exit(1);
      ...
      if (special_event > 0) {
    	StringBuilder sbstar = new StringBuilder(star);
...
//              ipl = Integer.parseInt(sastno) + 10000;
          ...
          if (ipl == SweConst.SE_FIXSTAR) {
            StringBuilder sstar=new StringBuilder(star);
            ...
            if (iflgret != SweConst.ERR && fmt.indexOf("=") >= 0) {
              ...
              try {
                ...
                iflgret = se.getRC();
                serr = new StringBuilder(se.getMessage());
              }
              attr[4] = mag;
            }
            se_pname=star;
...
//          if (strpbrk(fmt, "gGj") != NULL) { ... }
          if (fmt.indexOf("g")>=0 || fmt.indexOf("G")>=0 || fmt.indexOf("j")>=0) {
            ...
            if (hpos_meth >= 2 && Character.toLowerCase((char)ihsy) == 'g') {
              StringBuilder sstar2=new StringBuilder(star2);
  ...
  int print_line(int mode) {
    ...
    for (sp = fmt; c<fmt.length(); c++) {
      ...
      case 'T':
          ...
          System.out.print(String.format(Locale.US, "%02d.%02d.", jday, jmon, jyear));
          if (jut != 0) {
            int h, m, s;
            s = (int) (jut * 3600 + 0.5);
            h = (int) (s / 3600.0);
            m = (int) ((s % 3600) / 60.0);
            s %= 60;
	    System.out.print(String.format(Locale.US, " " + h + ":%02d:%02d", m, s));
            if (universal_time)
              System.out.print(" UT");
            else
              System.out.print(" ET");
...
//            }
  ...
  private int ut_to_lmt_lat(double t_ut, double[] geopos, double[] t_ret, StringBuilder serr) {
  ...
  private int call_rise_set(double t_ut, int ipl, StringBuilder star, int whicheph, int special_mode, double[] geopos, StringBuilder serr) {
    int ii;
    int rsmi = 0;
...
//          swe_revjul(tret[1], gregflag, &jyear, &jmon, &jday, &jut);
  ...
  private int call_lunar_eclipse(double t_ut, int whicheph, int special_mode, double[] geopos, StringBuilder serr) {
...
//        sprintf(sout + strlen(sout), "%2d.%02d.%04d\t%s\t%.4f/%.4f\tsaros %d/%d\t%.6f\tdt=%.2f\n", jday, jmon, jyear, hms(jut,BIT_LZEROES), attr[0],attr[1], (int) attr[9], (int) attr[10], t_ut, swe_deltat(t_ut) * 86400);
  ...
  private int call_solar_eclipse(double t_ut, int whicheph, int special_mode, double[] geopos, StringBuilder serr) {
    int i, ii, retc, eclflag, ecl_type = 0;
...
//      swe_revjul(tret[0], gregflag, &jyear, &jmon, &jday, &jut);
  ...
  private int call_lunar_occultation(double t_ut, int ipl, StringBuilder star, int whicheph, int special_mode, double[] geopos, StringBuilder serr) {
...
//    swe_revjul(dret[0], gregflag, &jyear, &jmon, &jday, &jut);
  ...
  private int call_heliacal_event(double t_ut, int ipl, StringBuilder star, int whicheph, int special_mode, double[] geopos, double[] datm, double[] dobs, StringBuilder serr) {
...
//              goto repeat_mercury;
  ...
  private int do_special_event(double tjd, int ipl, StringBuilder star, int special_event, int special_mode, double[] geopos, double[] datm, double[] dobs, StringBuilder serr) {
    int retc = 0;
  ...
  static void do_printf(StringBuilder info) {
   ...
   *   +                              on drive C:
   */
  private int make_ephemeris_path(int iflg, String argv0, StringBuilder opath) {	// path for output
    int sp;
    String path = "";
